...
- Access-mode literals, their derived TypeScript union, validation, and fallback behavior live in one shared policy module used by the DB, API, webhook decision layer, and web types.
- Actor extraction is event-specific: PR-open uses the PR author, while push and review-request events require the webhook sender. Missing event actors fail closed even in open mode.

## Validation

- Code Review TypeScript typecheck passes.
- Code Review unit tests pass (77 tests), including allowlist compatibility, open-mode behavior, blocked-user rejection, guardian override, and action-level fail-closed handling for missing push/request actors.
- The full monorepo typecheck and test suite pass (22 files, 138 tests), and the Code Review production bundle builds successfully.
...
- A migration-backed repository regression test creates both lists, switches modes through partial updates, reloads persisted state, and verifies that both lists and `manualTriggerAllowlist` survive intact.
